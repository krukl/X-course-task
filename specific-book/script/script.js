...
function sumPrice(){
    ...
    if (count.value > 25) {count.value = 25} else if (count.value < 0) {count.value = 0} }
totalPrice.innerHTML = priceId * count.value
// button "buy"
...
function hidesumPrice(){
    hidetotalPrice.innerHTML = hidepriceId * hidecount.value
     if (hidecount.value > 25) {hidecount.value = 25} else if (hidecount.value < 0) {hidecount.value = 0} }
   totalPrice.innerHTML = priceId * count.value
